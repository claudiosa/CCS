...
%%% SLD (Straight Line Distance) which serves as the Heuristic (h)
...
f(Rota, Objetivo,F) :-
    g(Rota, G),             /* calcula o custo g(n) */
    h(Rota, Objetivo, H),    /* calcula a funçao h(n) */
    F is (G+H) .            /* F = G + H */

/* using the SLD  for h function */
h([Nodo|_],Objetivo,H) :- distanceSL(Nodo,Objetivo,H).

/* cost from origin and its cost */
g([Nodo1,Nodo2],Custo):-
    getedge(Nodo1,Nodo2,Custo).
g([Nodo1, Nodo2 | Caminho],Custo):-
    getedge(Nodo1,Nodo2,Custo1),
