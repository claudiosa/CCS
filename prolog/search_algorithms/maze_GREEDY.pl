...
%%% swipl -q -t main -f maze_GREEDY.pl 
...
%%% KERNEL of SEARCH
search_GREEDY(X,Y,Path,Path) :-
           end_point(X,Y).
           %
           % format("\n FOUND A SOLUTION\n").

search_GREEDY(X0,Y0, PATH_TEMP, Path) :-
    
    expand_current_node(X0,Y0,L),
    heuristic_evalution(L, L_H),
    choice_the_best(L_H, (X1,Y1)),
    \+ member(  (X1,Y1) , PATH_TEMP ),
    search_GREEDY(X1,Y1,[ (X1,Y1)|PATH_TEMP],Path).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% end of KERNEL 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%% one_solution(Solution) :-
 main :-    
          start_point(X0,Y0),
          search_GREEDY(X0,Y0, [(X0,Y0)],Path),
