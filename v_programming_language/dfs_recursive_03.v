...
fn main() {
	...
	dfs_recursive(mut visited, graph_01, 'A', 'E', mut path) 
	print('\n Path ... all true: ${path} \n')
...
fn dfs_recursive(mut visited map[string]bool, graph map[string][]string, current string, target string, mut path []string)  {
	...
	for node in adjacent_nodes_not_visited {
		dfs_recursive(mut visited, graph, node, target, mut path)
	}
	//return 0
}
