...
fn main() {
	mut app := &App{
		lbl_elapsed_value: ui.label(text: '00.0s', text_size: 2.0 / 10)
		progress_bar: ui.progressbar(
			height: 20
			val: 0
			max: 100
		...
		layout: ui.column(
			...
			children: [
				ui.row(
					...
					children: [ui.label(text: 'Elapsed Time:', text_size: 2.0 / 10), app.progress_bar]
				),
				...
				ui.row(
					spacing: .1
					widths: [left, ui.stretch]
					children: [ui.label(text: 'Duration:', text_size: 2.0 / 10), app.slider]
				),
				ui.button(text: 'Reset', on_click: app.on_reset),
				ui.button(text: 'Start', on_click: app.on_start),
			]
		)
	)
	app.window = window
...
fn (mut app App) timer() {
	for {
		...
		//time_value.set_text('${math.ceil(app.elapsed_time * 100) / 100}s')
