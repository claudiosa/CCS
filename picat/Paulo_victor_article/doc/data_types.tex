...
\item \texttt{get($X$, $Key$) = $Val$}\index{\texttt{get/2}}: This function returns the \texttt{$Val$} of the key-value pair \texttt{$Key$$=$$Val$} that is attached to \texttt{$X$}. It throws an error if \texttt{$X$} has no attribute named $Key$.
\item \texttt{has\_key($X$, $Key$)}\index{\texttt{has\_key/2}}: This predicate is true if $X$ has an attribute named $Key$. 
\item \texttt{keys($X$) = $List$}\index{\texttt{keys/1}}: This function returns the list\index{list} of names of the attributes of $X$.
\item \texttt{put($X$, $Key$, $Val$)}\index{\texttt{put/3}}: This predicate attaches the key-value pair \texttt{$Key$$=$$Val$} to \texttt{$X$}, where \texttt{$Key$} is a non-variable term, and \texttt{$Val$} is any term. 
\item \texttt{put($X$, $Key$)}\index{\texttt{put/2}}: This predicate call is the same as \texttt{put($X$, $Key$,not\_a\_value)}.
\end{itemize}
...
\item \texttt{to\_oct\_string($Int$) = $String$}\index{\texttt{to\_oct\_string/1}}:  This function returns the octal representation of the integer\index{integer} $Int$ as a string\index{string}.
