...
int main ( void )
{
	...
	// OU
        ...
        MEU_TIPO vetor_pessoa[10];// 0 a 9

int i; // acesso via arquivo ./a.out < amostras_saude.txt 
for(i = 0; i<10; i++)
{
	printf("DIGITE SEU PESO in quilos Kg.gr: ");
	scanf("%f", &vetor_pessoa[i] . weight);

	printf("DIGITE SUA ALTURA in mts.cm: ");
	scanf("%f", &vetor_pessoa[i].height);

	calcula_IMC( &vetor_pessoa[i] );// eh ponteiro na funcao

	/* Output data to the user */
	imprime_SAIDA(vetor_pessoa[i]);
}
/* 
