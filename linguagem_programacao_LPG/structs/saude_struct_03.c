...
for(i = 0; i < 10; i++)
{
	printf("LEITURA ESTRANHA: PESO in quilos Kg.gr: ");
	scanf("%f", & (pt)-> weight);

	printf("LEITURA ESTRANHA: ALTURA in mts.cm: ");
	scanf("%f", & (pt)-> height);

	calcula_IMC( pt );// eh ponteiro na funcao
...
/* output data */
void imprime_SAIDA(MEU_TIPO p) // por valor
{
	printf("\nPESO (quilos): %2.2f\n", p.weight);
